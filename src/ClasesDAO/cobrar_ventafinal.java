...
public class cobrar_ventafinal {
    ...
    public boolean cobrar(double monto, float pagoefectivo, String referenciabanco, String formaPago, ArrayList productos, String impresionticket, double descuento, String referenciamixto, String formadescuento, String idclientefac, double ieps) {
        ...
        try {
            ...
            if (formaPago.equals("MIXTO")) {
                SQL = "INSERT INTO to_folios (no_folio, monto_total, efectivo, tarjeta,referencia_tarjeta, monto_puntos, no_puntos, fecha_movimiento,usuario_registro,tipo_movimiento,forma_pago,id_usuario,descuento,iva,cliente,ieps) "
                        + "VALUES ('" + folio + "', " + monto + ", " + pagoefectivo + ", " + referenciabanco + ", '" + referenciamixto + "', 0, 0, now(),'" + accesoSistema.nombreuser + "','VENTA','EFECTIVO','" + accesoSistema.iduser + "','" + resultdescuento + "','" + iva + "','" + idclientefac + "'," + ieps + ")";
            ...
            if (formaPago.equals("EFECTIVO")) {
                SQL = "INSERT INTO to_folios (no_folio, monto_total, efectivo, tarjeta,referencia_tarjeta, monto_puntos, no_puntos, fecha_movimiento,usuario_registro,tipo_movimiento,forma_pago,id_usuario,descuento,iva,cliente,ieps) "
                        + "VALUES ('" + folio + "', " + monto + ", " + pagoefectivo + ", 0, '" + referenciabanco + "', 0, 0, now(),'" + accesoSistema.nombreuser + "','VENTA','EFECTIVO','" + accesoSistema.iduser + "','" + resultdescuento + "','" + iva + "','" + idclientefac + "'," + ieps + ")";
                //System.out.println(""+SQL);
                stmt.executeUpdate(SQL);
            ...
            if (formaPago.equals("TARJETA")) {
                SQL = "INSERT INTO to_folios (no_folio, monto_total, efectivo, tarjeta,referencia_tarjeta, monto_puntos, no_puntos, fecha_movimiento,usuario_registro,tipo_movimiento,forma_pago,id_usuario,descuento,iva,cliente,ieps) "
                        + "VALUES ('" + folio + "', " + monto + ", 0," + monto + ", '" + referenciabanco + "', 0, 0, now(),'" + accesoSistema.nombreuser + "','VENTA','TARJETA','" + accesoSistema.iduser + "','" + resultdescuento + "','" + iva + "','" + idclientefac + "'," + ieps + ")";
                stmt.executeUpdate(SQL);
    ...
    public boolean cotizacion(float monto, float pagoefectivo, String referenciabanco, String formaPago, ArrayList productos, String impresionticket, double descuento, String tipoventa,
            String cliente, JComboBox clienteSeleccionado, JTextField costoEnvio,
            JComboBox factura, JDateChooser fechaPago,JComboBox comboFormaPago) {
 String arreCliente[]=clienteSeleccionado.getSelectedItem().toString().split("-");
        String clienteSeleccionado1 = arreCliente[0];
       
        String costoEnvio1 = costoEnvio.getText();
        String factura1 = factura.getSelectedItem().toString();
        String fechaPago1 = obtenerFecha(fechaPago);
        String formaPago1=comboFormaPago.getSelectedItem().toString();
      ...
      /*  Double valorIva=0.0;
        ...
        try {
            ...
            SQL = "INSERT INTO to_cotizacion (fecha, monto_total, usuario_registro,cliente,costoEnvio,factura,fechaPago,formaPago) "
                    + "VALUES (now(), " + monto + ", '" + accesoSistema.nombreuser + "','" + clienteSeleccionado1 + "','" + costoEnvio1 + "','" + factura1 + "','" + fechaPago1 + "','"+formaPago1+"')";
            stmt.executeUpdate(SQL);
            ...
            clienteSeleccionado.setSelectedItem("Selecciona...");
            costoEnvio.setText("");
            factura.setSelectedItem("Selecciona...");
            fechaPago.setDate(null);
            comboFormaPago.setSelectedItem("Selecciona...");
        } catch (SQLException se) {
            //Handle errors for JDBC
            se.printStackTrace();
            valida = false;
