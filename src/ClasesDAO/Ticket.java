...
public void ImprimirVentaFactura(String codigobarras,String tipoventa,float costotarjeta,String factura,String correo,String cliente,String idcliente,String tipopagos,String enviarcorreo){
	String encabezado="",footer="";
	conex cn = new conex();
	String sql="SELECT parametro from to_parametros where clave='ENCABEZADO'";
	System.out.println(sql);
	...
	try {
        ...
        sql="SELECT uuid, FechaTimbrado, selloCFD, noCertificadoSAT, selloSAT, FechaGeneracion from to_facturas where factura='"+factura+"'";
        System.out.println(""+sql);
	try {
            pstm100 = (PreparedStatement) cn.getConnection().prepareStatement(sql);
            ResultSet rs100 = (ResultSet) pstm100.executeQuery();
...
String a=encabezado;	   
	   ...
	   //a=a+" Productos: \n";
           ...
           pdffactura.imprimirfactura(factura,fechaGeneracion,FechaTimbrado,costoletra,montostr+"","0",ivatotal+"",total+"", uuid, noCertificadoSAT, selloCFD, selloSAT, cadenacomplemento,cliente,tipopagos,rfccliente,callereceptor,cpreceptor,ciudadreceptor,coloniareceptor,estadoreceptor,paramiva,ieps);
        //################ FIN GENERA FACTURA #############################
...
String prod="",nombremesa="";
            sql="SELECT Idtemp,cantidad,descripcion,Nombre from rest_temp_venta,rest_mesas where IdMesa=idrmesa and impreso=1 and idrmesa='"+mesa+"'";
            System.out.println(sql);            
            try {
                    pstm100 = (PreparedStatement) cn.getConnection().prepareStatement(sql);
                ...
                while (rs100.next()) {
                    nombremesa=rs100.getString("Nombre");
                     prod=prod+""+rs100.getString("cantidad")+" "+rs100.getString("descripcion")+"\n";           
